...
trait HasRoles{
    ...
    {
        $model_has_roles = config('privileges.table_names.model_has_roles');
            ...
            $q->when(PermissionRegistrar::$teams, function ($q) use ($model_has_roles) {
                ...
                $q->where($model_has_roles.'.'.PermissionRegistrar::$teamsKey, $teamId)
                    ->where(function ($q) use ($teamId) {
                        $teamField = config('privileges.table_names.roles').'.'.PermissionRegistrar::$teamsKey;
    ...
    public function scopeRole(Builder $query, $roles, $guard = null): Builder
    {
        ...
        return $query->whereHas('roles', function (Builder $subQuery) use ($roles) {
            $subQuery->whereIn(config('privileges.table_names.roles').'.id', \array_column($roles, 'id'));
        });
    }
